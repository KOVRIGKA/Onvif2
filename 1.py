...
def isNeedToStop(array):
	...
	print 'Is image ok? ' + str(need)
	return need


	# Подключение к камере
mycam = ONVIFCamera('192.168.15.43', 80, 'admin', 'Supervisor', 'C:\Python27\wsdl')
media_service = mycam.create_media_service()
media_profile = media_service.GetProfiles()[0]
imaging = mycam.create_imaging_service()
token = media_profile.VideoSourceConfiguration.SourceToken

# Ставим экспозицию и баланс белого в MANUAL
request = createImagingRequest(imaging, token)
try:
...
# Костыль для камер с большим шагом яркости
plusExposure = 0
minusExposure = 0
while True:
	...
	# Конвертируем изображение в цветовое пространство YCbCr
	ycbcr = cv2.cvtColor(im, cv2.COLOR_BGR2YCrCb)

	# Составляем гистограммы из 2 точек для цветности
	hist_cb_2 = cv2.calcHist([ycbcr],[2],None,[2],[0,256])
	hist_cr_2 = cv2.calcHist([ycbcr],[1],None,[2],[0,256])

	# Составляем гистограмму из 6 точек для яркости
	hist_y_6 = cv2.calcHist([ycbcr], [0], None, [6], [0,256])

	# Составляем гистограммы из 256 точек
	...
	if not isNeedToStop(hist_y):
		...
		if black and white:
			# Cлева и справа слишком большие значения, уменьшаем контрастность
			...
			# Cлишком большое значение только слева, увеличиваем яркость
	...
	# Некоторые камеры не сразу обновляют snapshot
	time.sleep(1)
